...
export function AdminCarousel() {
  ...
  const fetchCarouselImages = async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://localhost:3001/api/carousel/admin/all');
  ...
  const openDialog = (image?: CarouselImage) => {
    if (image) {
      ...
      setImagePreviews([`http://localhost:3001${image.image_path}`]);
      setSelectedFiles([]);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingImage) {
      ...
      try {
        const response = await fetch(`http://localhost:3001/api/carousel/${editingImage.id}`, {
          method: 'PUT',
          body: submitFormData,
        });
        ...
        <Dialog open={!!previewImage} onOpenChange={() => setPreviewImage(null)}>
          <DialogContent className="max-w-2xl">
            ...
            {previewImage && (
              <div>
                <img
                  src={`http://localhost:3001${previewImage.image_path}`}
