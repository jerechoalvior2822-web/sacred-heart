...
export function AdminSouvenirs() {
  ...
  const fetchSouvenirs = async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://localhost:3001/api/souvenirs');
  ...
  const handleEdit = (item: SouvenirItem) => {
    ...
    setImagePreview(item.image ? `http://localhost:3001${item.image}` : '');
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/souvenirs/${id}`, {
        method: 'DELETE',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingItem ? `http://localhost:3001/api/souvenirs/${editingItem.id}` : 'http://localhost:3001/api/souvenirs';
