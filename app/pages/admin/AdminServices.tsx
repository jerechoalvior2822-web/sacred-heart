...
export function AdminServices() {
  ...
  const fetchBookedDates = async (serviceId?: number) => {
    try {
      const url = serviceId 
        ? `http://localhost:3001/api/bookings/booked-dates?serviceId=${serviceId}`
        : 'http://localhost:3001/api/bookings/booked-dates';
  ...
  const handleEdit = (service: ServiceItem) => {
    ...
    setSelectedImageFile(null);
    setImagePreview(service.image?.startsWith('/assets') ? `http://localhost:3001${service.image}` : service.image);
                ...
                <Card hover>
                  {service.image ? (
                    <div className="aspect-video overflow-hidden">
                      <img
                        src={service.image.startsWith('/assets') ? `http://localhost:3001${service.image}` : service.image}
                        alt={service.name}
