...
export function AdminMassSchedules() {
  ...
  const fetchAssignments = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3001/api/mass-schedules');
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this mass assignment?')) {
      try {
        const response = await fetch(`http://localhost:3001/api/mass-schedules/${id}`, {
          method: 'DELETE',
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingAssignment) {
        const response = await fetch(`http://localhost:3001/api/mass-schedules/${editingAssignment.id}`, {
          method: 'PUT',
