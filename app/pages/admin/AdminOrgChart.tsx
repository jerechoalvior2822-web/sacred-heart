...
export function AdminOrgChart() {
  ...
  const fetchOrgMembers = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/org-members');
  ...
  const handleDelete = (id: number) => {
    if (confirm('Are you sure you want to remove this member from the org chart?')) {
      fetch(`http://localhost:3001/api/org-members/${id}`, {
        method: 'DELETE',
      })
        .then(res => {
          if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMember) {
        const response = await fetch(`http://localhost:3001/api/org-members/${editingMember.id}`, {
          method: 'PUT',
