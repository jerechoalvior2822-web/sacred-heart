...
export function Bookings() {
  ...
  const handleUpload = async (bookingId: number) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/bookings/upload-documents', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleViewDocument = (docPath: string) => {
    try {
      ...
      if (docPath.startsWith('/assets')) {
        fullPath = `http://localhost:3001${docPath}`;
      } else if (!docPath.startsWith('http')) {
        fullPath = `http://localhost:3001/assets/uploads/documents/${docPath}`;
      }
