...
const getImageUrl = (image: string) => {
  ...
  if (image.startsWith('/')) return `http://localhost:3001${image}`;
  if (image.startsWith('data:')) return image;
  if (image.startsWith('http')) return image;
  return `http://localhost:3001/assets/uploads/${image}`;
};
...
export function Announcements() {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/announcements${query}`);
  ...
  const handleLike = async (postId: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/announcements/${postId}/like${query}`, {
        method,
  ...
  const handleComment = async (postId: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/announcements/${postId}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: user.id,
  ...
  const handleCommentLike = async (postId: number, commentId: number, currentlyLiked: boolean) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/announcements/comments/${commentId}/like${query}`, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: currentlyLiked ? undefined : JSON.stringify({ userId: user.id }),
      });
  ...
  const handleDeleteComment = async (postId: number, commentId: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/announcements/${postId}/comments/${commentId}?user_id=${encodeURIComponent(user.id)}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok) {
  ...
  const fetchAnnouncementComments = async (postId: number) => {
    try {
      const query = user?.id ? `?user_id=${encodeURIComponent(user.id)}` : '';
      const response = await fetch(`http://localhost:3001/api/announcements/${postId}/comments${query}`);
      if (!response.ok) {
